export default function ColorSection({
  ...
  return (
    <section
      className={`rounded-[10px] p-4 lg:p-6 md:self-start lg:rounded-[20px] xl:rounded-[40px] ${panelBg} flex flex-col gap-4 h-full`}
      style={{
    ...
    >
      ...
      <div className="flex flex-wrap gap-2 h-full items-center justify-center lg:justify-start">
        {colorThemes.map((t) => (
          <button
            key={t.id}
            type="button"
            ...
            aria-label={t.id}
            className="h-8 w-8 shrink-0 cursor-pointer rounded-full transition-transform hover:scale-110"
            style={{
              background: t.swatch,
